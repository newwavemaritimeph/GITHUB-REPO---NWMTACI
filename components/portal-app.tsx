...
const nav: { label: Module; icon: string; roles?: Role[] }[] = [
  ...
  { label: "Enrollments", icon: "▤", roles: ["Admin", "Registration", "Cashier", "Accounting", "Training Operations"] },
  { label: "Courses & centers", icon: "◇", roles: ["Admin", "Registration", "Accounting", "Training Operations"] },
  { label: "Schedules", icon: "□", roles: ["Admin", "Registration", "Training Operations", "Instructor"] },
  { label: "Payments", icon: "₱", roles: ["Admin", "Cashier", "Accounting", "Registration"] },
  ...
  { label: "Requests", icon: "↗" },
  { label: "HR & payroll", icon: "♙", roles: ["Admin", "HR"] },
  { label: "Reports", icon: "↥", roles: ["Admin", "Registration", "Cashier", "Accounting", "Training Operations", "HR"] },
...
function Dashboard({ role, go }: { role: Role; go: (module: Module) => void }) {
  ...
  const metrics: Record<Role, { label: string; value: string; note: string; icon: string; module: Module }[]> = {
    Admin: [
      ...
      { label: "Collections today", value: pesos(collectionsToday), note: `${pendingPayments.length} to verify`, icon: "₱", module: "Payments" },
      ...
      { label: "Available slots", value: String(openSeats), note: `${state.batches.filter((batch) => batch.status === "Open").length} open batches`, icon: "□", module: "Schedules" },
    ...
    Registration: [
      ...
      { label: "Available slots", value: String(openSeats), note: "Published batches", icon: "□", module: "Schedules" },
      { label: "Pending requests", value: String(pendingRequests.length), note: "Awaiting a decision", icon: "↗", module: "Requests" },
    ],
    Cashier: [
